...
exports.makeVectorsConfig = exports.parseVectorizerConfig = exports.parseVectorIndex = exports.classToCollection = exports.resolveReference = exports.resolveProperty = exports.ReferenceTypeGuards = void 0;
...
const parseVectorIndex = (module) => {
    if (module.config === undefined)
        return undefined;
    if (parsing_js_1.VectorIndexGuards.isDynamic(module.config)) {
        const { hnsw, flat, ...conf } = module.config;
        return {
            ...conf,
            hnsw: (0, exports.parseVectorIndex)({ name: 'hnsw', config: hnsw }),
            flat: (0, exports.parseVectorIndex)({ name: 'flat', config: flat }),
        };
    }
    let multivector;
    if (parsing_js_1.VectorIndexGuards.isHNSW(module.config) && module.config.multiVector !== undefined) {
        multivector = {
            aggregation: module.config.multiVector.aggregation,
            enabled: true,
        };
        if (module.config.multiVector.encoding !== undefined &&
            parsing_js_1.MultiVectorEncodingGuards.isMuvera(module.config.multiVector.encoding)) {
            multivector.muvera = {
                enabled: true,
    ...
    const { quantizer, ...rest } = module.config;
    const conf = {
        ...rest,
        multivector,
    };
    ...
    if (parsing_js_1.QuantizerGuards.isSQCreate(quantizer)) {
        const { type, ...quant } = quantizer;
        return {
            ...conf,
            sq: {
    ...
    if (parsing_js_1.QuantizerGuards.isRQCreate(quantizer)) {
        const { type, ...quant } = quantizer;
        return {
            ...conf,
            rq: {
    ...
    if (parsing_js_1.QuantizerGuards.isUncompressedCreate(quantizer)) {
        return {
            ...conf,
...
exports.parseVectorIndex = parseVectorIndex;
const parseVectorizerConfig = (config) => {
    if (config === undefined)
        return {};
    const { vectorizeCollectionName, ...rest } = config;
    return {
        ...rest,
        vectorizeClassName: vectorizeCollectionName,
    };
};
exports.parseVectorizerConfig = parseVectorizerConfig;
const makeVectorsConfig = (configVectorizers) => {
    ...
    vectorizersConfig.forEach((v) => {
        const vectorConfig = {
            vectorIndexConfig: (0, exports.parseVectorIndex)(v.vectorIndex),
        ...
        vectorConfig.vectorizer[vectorizer] = {
            ...
            ...(0, exports.parseVectorizerConfig)(v.vectorizer.config),
        };
        if (v.name === undefined) {
            throw new errors_js_1.WeaviateInvalidInputError('vectorName is required for each vectorizer when specifying more than one vectorizer');
        }
        vectorsConfig[v.name] = vectorConfig;
    });
    return { vectorsConfig, vectorizers };
};
exports.makeVectorsConfig = makeVectorsConfig;
...
class ConfigMapping {
    ...
    static namedVectors(v) {
        ...
        Object.keys(v).forEach((key) => {
            ...
            if (!populated(vectorizer))
                throw new errors_js_1.WeaviateDeserializationError(`Vectorizer was not returned by Weaviate for ${key} named vector`);
            const vectorizerNames = Object.keys(vectorizer);
            if (vectorizerNames.length !== 1)
                throw new errors_js_1.WeaviateDeserializationError(`Expected exactly one vectorizer for ${key} named vector, got ${vectorizerNames.length}`);
            const vectorizerName = vectorizerNames[0];
            const { properties, ...restA } = vectorizer[vectorizerName] || {};
    ...
    static invertedIndex(v) {
        ...
        if (!populated(v.cleanupIntervalSeconds))
            throw new errors_js_1.WeaviateDeserializationError('Inverted index cleanup interval was not returned by Weaviate');
        return {
    ...
    static sharding(v) {
        ...
        if (!exists(v.desiredCount))
            throw new errors_js_1.WeaviateDeserializationError('Sharding desired count was not returned by Weaviate');
        if (!exists(v.actualCount))
            throw new errors_js_1.WeaviateDeserializationError('Sharding actual count was not returned by Weaviate');
        if (!exists(v.desiredVirtualCount))
            throw new errors_js_1.WeaviateDeserializationError('Sharding desired virtual count was not returned by Weaviate');
        if (!exists(v.actualVirtualCount))
            throw new errors_js_1.WeaviateDeserializationError('Sharding actual virtual count was not returned by Weaviate');
        if (!exists(v.key))
    ...
    static pqEncoder(v) {
        ...
        if (!exists(v.distribution))
            throw new errors_js_1.WeaviateDeserializationError('PQ encoder distribution was not returned by Weaviate');
        return {
            type: v.type,
    ...
    static vectorIndexHNSW(v) {
        ...
        if (!exists(v.cleanupIntervalSeconds))
            throw new errors_js_1.WeaviateDeserializationError('Vector index cleanup interval was not returned by Weaviate');
        if (!exists(v.distance))
            throw new errors_js_1.WeaviateDeserializationError('Vector index distance was not returned by Weaviate');
        if (!exists(v.dynamicEfMin))
            throw new errors_js_1.WeaviateDeserializationError('Vector index dynamic ef min was not returned by Weaviate');
        if (!exists(v.dynamicEfMax))
            throw new errors_js_1.WeaviateDeserializationError('Vector index dynamic ef max was not returned by Weaviate');
        if (!exists(v.dynamicEfFactor))
            throw new errors_js_1.WeaviateDeserializationError('Vector index dynamic ef factor was not returned by Weaviate');
        if (!exists(v.ef))
        ...
        if (!exists(v.efConstruction))
            throw new errors_js_1.WeaviateDeserializationError('Vector index ef construction was not returned by Weaviate');
        if (!exists(v.flatSearchCutoff))
            throw new errors_js_1.WeaviateDeserializationError('Vector index flat search cut off was not returned by Weaviate');
        if (!exists(v.maxConnections))
            throw new errors_js_1.WeaviateDeserializationError('Vector index max connections was not returned by Weaviate');
        if (!exists(v.skip))
        ...
        if (!exists(v.vectorCacheMaxObjects))
            throw new errors_js_1.WeaviateDeserializationError('Vector index vector cache max objects was not returned by Weaviate');
        let quantizer;
    ...
    static multiVector(v) {
        if (!exists(v.enabled))
            throw new errors_js_1.WeaviateDeserializationError('Multi vector enabled was not returned by Weaviate');
        if (v.enabled === false)
            return undefined;
        if (!exists(v.aggregation))
            throw new errors_js_1.WeaviateDeserializationError('Multi vector aggregation was not returned by Weaviate');
        let encoding;
        if (exists(v.muvera)) {
    ...
    static rq(v) {
        if (v === undefined)
            throw new errors_js_1.WeaviateDeserializationError('RQ was not returned by Weaviate');
        if (!exists(v.enabled))
            throw new errors_js_1.WeaviateDeserializationError('RQ enabled was not returned by Weaviate');
        if (v.enabled === false)
            return undefined;
    ...
    static sq(v) {
        if (v === undefined)
            throw new errors_js_1.WeaviateDeserializationError('SQ was not returned by Weaviate');
        if (!exists(v.enabled))
            throw new errors_js_1.WeaviateDeserializationError('SQ enabled was not returned by Weaviate');
        if (v.enabled === false)
            return undefined;
        const rescoreLimit = v.rescoreLimit === undefined ? 1000 : v.rescoreLimit;
    ...
    static vectorIndexDynamic(v) {
        ...
        if (!exists(v.threshold))
            throw new errors_js_1.WeaviateDeserializationError('Vector index threshold was not returned by Weaviate');
        if (!exists(v.distance))
            throw new errors_js_1.WeaviateDeserializationError('Vector index distance was not returned by Weaviate');
        if (!exists(v.hnsw))
    ...
    static properties(v) {
        ...
        return v
            ...
            if (!populated(prop.dataType))
                throw new errors_js_1.WeaviateDeserializationError('Property data type was not returned by Weaviate');
    ...
    static references(v) {
        ...
        return v
            .filter((prop) => {
            if (!populated(prop.dataType))
                throw new errors_js_1.WeaviateDeserializationError('Reference data type was not returned by Weaviate');
